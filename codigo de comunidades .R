...
# Top 9 comunidades más grandes
cat("\nTop 9 comunidades más grandes:\n")
top_9 <- head(sort(tamanos, decreasing = TRUE), 9)
print(top_9)
...
cat(" Top 9 modulos mas grandes :\n")

print(head(tabla_tutor, 9))

cat("\n Top 9 modulos mas conectados:\n")

top_conectados <- tabla_tutor %>% arrange(desc(Enlaces)) %>% head(9)
print(top_conectados)

cat("\n Top 9 modulos mas densos:\n")

top_densos <- tabla_tutor %>% arrange(desc(Densidad)) %>% head(9)
print(top_densos)


write.csv(top_conectados, file = "/Users/alejandropintacastro/Desktop/top_conectados.csv", row.names = FALSE)
